...
App.controller("refundManageController", function ($scope, ngProgressFactory, restful, $rootScope, $uibModal, toastr) {
    ...
    $scope.query = function () {
        $scope.data.lastLoginTimeStart = new Date($scope.data.startDate).setHours("00", "00", "00");
        $scope.data.lastLoginTimeEnd = new Date($scope.data.endDate).setHours("23", "59", "59");
        $scope.progressbar.start();
        $scope.refundPromise = restful.fetch($rootScope.api.getLsrefundlist, "POST", $scope.data).then(function(res) {
